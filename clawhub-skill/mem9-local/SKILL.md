...
version: 1.0.0
description: |
   Persistent cloud memory for OpenClaw agents (local dev).
...
```bash
curl -sX POST http://192.168.122.97:8080/v1alpha1/mem9s | jq .
```
...
jq --arg tid "$TENANT_ID" '
  ...
  .plugins.entries.mem9 = {
    enabled: true,
    config: { apiUrl: "http://192.168.122.97:8080", tenantID: $tid }
  } |
  ...
  "plugins": {
    ...
    "entries": {
      "mem9": {
        "enabled": true,
        "config": {
          "apiUrl": "http://192.168.122.97:8080",
...
export API="http://192.168.122.97:8080/v1alpha1/mem9s/$TENANT_ID"
```
...
| `404` on API call | Verify tenantID; run `curl http://192.168.122.97:8080/healthz` |
---
...
3. **Explore** — `curl -s "http://192.168.122.97:8080/v1alpha1/mem9s/$TENANT_ID/memories?limit=10" | jq .`
